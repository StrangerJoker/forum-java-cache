...
public class IndexController {
    ...
    public String index(IndexRequest request, Model model,
        ...
        // 查询 加载配置信息
        ResultModel<List<ConfigResponse>> configResult =
                configApiService.queryAvailable(Sets.newHashSet(ConfigTypeEn.HOME_CAROUSEL.getValue()
                , ConfigTypeEn.SIDEBAR_CAROUSEL.getValue()));
        // 配置信息可用 则 进行页面的配置
        if (configResult.getSuccess() && !ObjectUtils.isEmpty(configResult.getData())) {
            model.addAttribute("homeCarouselList", webUtil.carouselList(configResult.getData(),
                    ConfigTypeEn.HOME_CAROUSEL));
            model.addAttribute("sideCarouselList", webUtil.carouselList(configResult.getData(),
                    ConfigTypeEn.SIDEBAR_CAROUSEL));
        } else {
            model.addAttribute("homeCarouselList", new ArrayList<>());
            model.addAttribute("sideCarouselList", new ArrayList<>());
        }

        // 侧边栏 热点问题 类型信息 和使用的tag
    ...
    private List<Map<String, Object>> typeList(String typeName) {
        ...
        pageRequestModel.setFilter(ArticleUserPageRequest.builder()
                .typeName(typeName)
                .build());
        ResultModel<PageResponseModel<ArticleUserPageResponse>> resultModel =
                articleApiService.userPage(pageRequestModel);
        if (!resultModel.getSuccess()) {
            return postsList;
        }

        // 保存 文章的 id title createAt
        PageResponseModel<ArticleUserPageResponse> pageResponseModel = resultModel.getData();
